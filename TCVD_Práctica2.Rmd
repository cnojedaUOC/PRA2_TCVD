...
En caso de que hubieran elementos nulos se puede, entre otros: 
...
En la siguiente imagen se pueden observar los valores extremos (__outliers__) de cada atributo:

```{r message= FALSE, warning=FALSE}

boxplot(scale(breast_data_def[2:5]), xlab ="variables", cex.axis=0.75, ylab = "Rango de Valores", col="blue", main="Información sobre anomalías y valores extremos")
boxplot(scale(breast_data_def[6:10]), xlab ="variables", cex.axis=0.55, ylab = "Rango de Valores", col="blue", main="Información sobre anomalías y valores extremos")
boxplot(scale(breast_data_def[11:15]), xlab ="variables", cex.axis=0.60, ylab = "Rango de Valores", col="blue", main="Información sobre anomalías y valores extremos")
boxplot(scale(breast_data_def[16:20]), xlab ="variables", cex.axis=0.60, ylab = "Rango de Valores", col="blue", main="Información sobre anomalías y valores extremos")
boxplot(scale(breast_data_def[21:25]), xlab ="variables", cex.axis=0.60, ylab = "Rango de Valores", col="blue", main="Información sobre anomalías y valores extremos")
boxplot(scale(breast_data_def[26:31]), xlab ="variables", cex.axis=0.50, ylab = "Rango de Valores", col="blue", main="Información sobre anomalías y valores extremos")
```

Se puede observar que en todos los gráficos hay datos atípicos, lo que no quiere decir que sean erróneos. Si los valores atípicos corresponden con tumores malignos, pueden ser valores correctos y no errores en las medidas. Posteriormente se comprobará que la mayoría de estos valores __outliers__ corresponden con tumores malignos.
...
A continuación se va a realizar el análisis descriptivo y estadístico de los datos. En el primero de ellos se van a analizar la media, mediana y desviación estándar de las muestras. En las gráficos de cajas presentados anteriormente, se puede apreciar que los valores de la mediana de los atributos están muy cerca de la mitad de la caja, indicando que los valores de los datos son más o menos simétricos. En el análisis descriptivo se puede comprobar que los atributos no están todos en el mismo rango, existiendo en algunos de ellos, una gran diferencia entre los valores mínimo y máximo:

```{r message= FALSE, warning=FALSE}
summary(breast_data_def_grouping)

```

Por otro lado, el análisis estadístico inferencial tiene por objetivo modelar los datos a través de una distribución conocida. Partiendo de la premisa que el conjunto de datos estudiado representa una fracción de la totalidad de una población, su objetivo es inferir cómo es esa población, asumiendo un grado de error en las estimaciones por el hecho de disponer de una muestra reducida de los datos. En el siguiente subapartado se comprobará la normalidad y homogeneidad de la varianza.
...
Si analizamos gráficamente la relación del atributo **Symmetry_worst** con la variable objetivo:

```{r message= FALSE, warning=FALSE}
if (!require('ggplot2')) install.packages('ggplot2'); library('ggplot2')

ggplot(data=breast_data_def_grouping,aes(x=Diagnosis,y=Symmetry_worst,fill=Diagnosis))+geom_boxplot()+ggtitle("Peor simetría de célual en el diagnóstico Benigno vs Maligno") 
```

También se observa cómo la varianza es mayor en los casos de malignidad, aunque la mediana está centrada en ambos diagnósticos. Existen muchos valores atípicos.

Si por ejemplo comprobamos un atributo que no está en la lista anterior, como **Texture_worst**:


```{r message= FALSE, warning=FALSE}
if (!require('ggplot2')) install.packages('ggplot2'); library('ggplot2')

ggplot(data=breast_data_def_grouping,aes(x=Diagnosis,y=Texture_worst,fill=Diagnosis))+geom_boxplot()+ggtitle("Peor textura de la célula en el diagnóstico Benigno vs Maligno") 
```
...
Puesto que las variables no se ajustan a una distribucion normal, se empleará el coecifiente de correlación de **Spearman** para comprobar qué variables muestran una mayor relación con el tipo de tumor (benigno o maligno): 
...
```
Al ser __p_value__ menor que el valor de la significancia (0.05), se rechaza la hipótesis nula, por tanto, la media del primer grupo de muestras  debe ser mayor (alternative es 'g' o 'greater') a la media del segundo grupo de muestras y si el primer grupo de muestras pertenece a tumores malignos (valor 2), tiene un valor asignado mayor al que tendrían el otro grupo en el que habría tumores benignos (valor 1) y malignos (valor 2). Esto quiere decir que podemos afirmar que el primer grupo de muestras está formado por muestras que corresponden con tumores malignos, es decir, los valores __outliers__ (que forman completamente ese grupo) están en muestras con tumores malignos, luego son valores válidos. En el apartado final sobre resultados, se presenta de forma gráfica cómo los valores __outliers__ pertenecen mayoritariamente a muestras de tumores malignos.

En función de los análisis anteriores, se seleccionan las  varibles del conjunto de datos teniendo en cuenta cuáles de ellas están más correladas con la variable objetivo. Estas son:  **Perimeter_worst**, **Concavity_mean**, **Area_se**, **Concavity_worst** y la variable objetivo **Diagnosis**. Así:

```{r message= FALSE, warning=FALSE}

# se seleccionan las variables mejor correladas con la variable objetivo del grupo inicial
data_to_analyze <- breast_data_def_grouping[c('Diagnosis', 'Perimeter_worst', 'Concavity_worst', 'Concavity_mean', 'Area_se')]
...
Se observa que de 427 mujeres del grupo de entrenamiento, con el modelo usado se han clasificado correctamente 408 (260 verdaderos positivos y 148 verdaderos negativos). Asimismo hay 8 falsos negativos (es decir, 8 mujeres que no tenían cáncer de mama que se predijeron que lo tenían) y 11 falsos positivos (es decir, 11 mujeres que tenían cáncer de mama y que se predijeron que no lo tenían). El clasificador tienen una precisión de 95% (mayor al 62% que es el número de mujeres sin cáncer de mama). Por otro lado, el clasificador tiene una sensibilidad del 97% (verdaderos positivos identificados).
...
Se observa que de 142 mujeres del grupo de test, con el modelo usado se han clasificado correctamente 133 (84 verdaderos positivos y 49 verdaderos negativos). Asimismo hay 5 falsos negativos (es decir, 5 mujeres que no tenían cáncer de mama que se predijeron que lo tenían) y 4 falsos positivos (es decir, 4 mujeres que tenían cáncer de mama y que se predijeron que no lo tenían). El clasificador tienen una precisión de 94% (mayor al 62% que es el número de mujeres sin cáncer de mama). Por otro lado, el clasificador tiene una sensibilidad del 94% (verdaderos positivos identificados).
...
Se observa que de 427 mujeres del grupo de entrenamiento, con el modelo usado se han clasificado correctamente a 396. El clasificador tienen una precisión del 93% (mayor al 62% que es el número de mujeres sin cáncer de mama). Por otro lado, el clasificador tiene una sensibilidad del 96% (verdaderos positivos identificados).
...
Se observa que de 142 mujeres del grupo de entrenamiento, con el modelo usado se han clasificado correctamente a 128. Ha habido 10 falsos positivos y 4 falsos negativos. El clasificador tienen una precisión de 90% (mayor al 62% que es el número de mujeres sin cáncer de mama). Por otro lado, el clasificador tiene una sensibilidad del 95% (verdaderos positivos identificados).
...
Se observa que de 427 mujeres del grupo de entrenamiento, con el modelo usado se han clasificado correctamente a 403. El clasificador tiene una precisión de 94% (mayor al 62% que es el número de mujeres sin cáncer de mama). Por otro lado, el clasificador tiene una sensibilidad del 96% (verdaderos positivos identificados).
...
Se observa que de 142 mujeres del grupo de entrenamiento, con el modelo usado se han clasificado correctamente a 131. Ha habido 5 falsos positivos y 6 falsos negativos. El clasificador tienen una precisión de 92% (mayor al 62% que es el número de mujeres sin cáncer de mama). Por otro lado, el clasificador tiene una sensibilidad del 93% (verdaderos positivos identificados).
...
Se observa que de 142 mujeres del grupo de test, con el modelo usado se han clasificado correctamente 133 (85 verdaderos positivos y 48 verdaderos negativos). Asimismo hay 4 falsos negativos (es decir, 4 mujeres que no tenían cáncer de mama que se predijeron que lo tenían) y 5 falsos positivos (es decir, 5 mujeres que tenían cáncer de mama y que se predijeron que no lo tenían). El clasificador tienen una precisión de 94% (mayor al 62% que es el número de mujeres sin cáncer de mama). Por otro lado, el clasificador tiene una sensibilidad del 95% (verdaderos positivos identificados).
...
Observándose que el mejor clasificador es el modelo de **Regresión logística** con un 94% de precisión (un poco mayor que en la prueba previa), frente a un 92% del modelo **Árbol de decisión**. 
...
Así, para comprobar la calidad del atributo **Perimeter_worst**:
...
Se observa que las mujeres con diagnóstico benigno tienen un valor de **Perimeter_worst** inferior a la media (aproximadamente 110), mientras que las que tienen un diagnóstico maligno, el valor está por encima de la media:

```{r  message= FALSE, warning=FALSE}
datos_PW_menos100_B <- filter(breast_data, breast_data$Perimeter_worst <= 110 & breast_data$Diagnosis == "B")
total_B <- filter(breast_data, breast_data$Diagnosis == "B")
valorB <- (nrow(datos_PW_menos100_B)/nrow(total_B))*100
cat("El porcentaje de mujeres con valor inferior a la media en el atributo peor perímetro (Perimeter_worst) y diagnóstico benigno es: " , valorB , "%\n")

datos_PW_menos100_M <- filter(breast_data, breast_data$Perimeter_worst <= 110 & breast_data$Diagnosis == "M")
total_M <- filter(breast_data, breast_data$Diagnosis == "M")
valorM <- (nrow(datos_PW_menos100_M)/nrow(total_M))*100
cat("El porcentaje de mujeres con valor inferior a la media en el atributo peor perímetro (Perimeter_worst) y diagnóstico maligno es: " , valorM , "%")


```

Si analizamos gráficamente el atributo **Concavity_mean**:


```{r message= FALSE, warning=FALSE}
if (!require('ggplot2')) install.packages('ggplot2'); library('ggplot2')

ggplot(breast_data, aes(x = Concavity_mean)) + geom_histogram(binwidth = 0.01, col='black', fill='blue') + facet_wrap(~ Diagnosis)+ggtitle("Media de la concavidad para diagnósticos Benignos vs Malignos") +labs(x="Media de área", y="Número de mujeres") +
  geom_vline(aes(xintercept = mean(Concavity_mean), colour="media"),
              linetype = "dashed",
              size = 1) 
```

Se observa que las mujeres con diagnóstico benigno tienen un valor de **Concavity_mean** mayoritariamente inferior a la media (aproximadamente 0.08), mientras que las que tienen un diagnóstico maligno, mayoritariamente tiene un valor del atributo por encima de la media:

```{r  message= FALSE, warning=FALSE}
datos_CM_B <- filter(breast_data, breast_data$Concavity_mean <= 0.08 & breast_data$Diagnosis == "B")
total_CM_B <- filter(breast_data, breast_data$Diagnosis == "B")
valorCMB <- (nrow(datos_CM_B)/nrow(total_CM_B))*100
cat("El porcentaje de mujeres con valor inferior a la media en el atributo Concavity_mean y diagnóstico benigno es: " , valorCMB , "%\n")

datos_CM_M <- filter(breast_data, breast_data$Concavity_mean <= 0.08 & breast_data$Diagnosis == "M")
total_CM_M <- filter(breast_data, breast_data$Diagnosis == "M")
valorCMM <- (nrow(datos_CM_M)/nrow(total_CM_M))*100
cat("El porcentaje de mujeres con valor inferior a la media en el atributo Concavity_mean y diagnóstico maligno es: " , valorCMM , "%")

```

Si analizamos gráficamente el valor del atributo **Area_se**:


```{r message= FALSE, warning=FALSE}
if (!require('ggplot2')) install.packages('ggplot2'); library('ggplot2')

ggplot(breast_data, aes(x = Area_se)) + geom_histogram(binwidth = 20, col='black', fill='blue') + facet_wrap(~ Diagnosis)+ggtitle("Media de área para diagnósticos Benignos vs Malignos") +labs(x="Media de área", y="Número de mujeres") +
  geom_vline(aes(xintercept = mean(Area_se), colour="media"),
              linetype = "dashed",
              size = 1) 
```
 
Se observa que las mujeres con diagnóstico benigno tienen un valor de **Area_se** mayoritariamente inferior a la media (aproximadamente 40), mientras que las que tienen un diagnóstico maligno, mayoritariamente tiene un valor del atributo por encima de la media:

```{r  message= FALSE, warning=FALSE}
datos_ASE_B <- filter(breast_data, breast_data$Area_se <= 40 & breast_data$Diagnosis == "B")
total_ASE_B <- filter(breast_data, breast_data$Diagnosis == "B")
valorASEB <- (nrow(datos_ASE_B)/nrow(total_ASE_B))*100
cat("El porcentaje de mujeres con valor inferior a la media en el atributo Area_se y diagnóstico benigno es: " , valorASEB , "%\n")

datos_ASE_M <- filter(breast_data, breast_data$Area_se <= 40 & breast_data$Diagnosis == "M")
total_ASE_M <- filter(breast_data, breast_data$Diagnosis == "M")
valorASEM <- (nrow(datos_ASE_M)/nrow(total_ASE_M))*100
cat("El porcentaje de mujeres con valor inferior a la media en el atributo Area_se y diagnóstico maligno es: " , valorASEM , "%")

```

Si analizamos gráficamente el atributo **Concavity_worst**:
...
Se observa que las mujeres con diagnóstico benigno tienen un valor de **Concavity_worst** mayoritariamente inferior a la media (aproximadamente 0.27), mientras que las que tienen un diagnóstico maligno, mayoritariamente tiene un valor del atributo por encima de la media:

```{r  message= FALSE, warning=FALSE}
datos_CW_B <- filter(breast_data, breast_data$Concavity_worst <= 0.27 & breast_data$Diagnosis == "B")
total_CW_B <- filter(breast_data, breast_data$Diagnosis == "B")
valorCWB <- (nrow(datos_CW_B)/nrow(total_CW_B))*100
cat("El porcentaje de mujeres con valor inferior a la media en el atributo Concavity_worst y diagnóstico benigno es: " , valorCWB , "%\n")

datos_CW_M <- filter(breast_data, breast_data$Concavity_worst <= 0.27 & breast_data$Diagnosis == "M")
total_CW_M <- filter(breast_data, breast_data$Diagnosis == "M")
valorcwM <- (nrow(datos_CW_M)/nrow(total_CW_M))*100
cat("El porcentaje de mujeres con valor inferior a la media en el atributo Concavity_worst y diagnóstico maligno es: " , valorcwM , "%")
...
ggplot(datos_pw, aes(x=Diagnosis,  y=Total, fill=Diagnosis))+ ggtitle("Diagnóstico de los outliers del atributo Perimeter_worst") +
...
ggplot(datos_cm, aes(x=Diagnosis,  y=Total, fill=Diagnosis))+ ggtitle("Diagnóstico de los outliers del atributo Concavity_mean") +
  geom_bar(stat='identity') +                                         
  geom_text(aes(label=Total), vjust=1.6, color="black", position = position_dodge(0.9),  size=4.0) 
...
ggplot(datos_ase, aes(x=Diagnosis,  y=Total, fill=Diagnosis))+ ggtitle("Diagnóstico de los outliers del atributo Area_se") +
  geom_bar(stat='identity') +                                         
  geom_text(aes(label=Total), vjust=1.6, color="black", position = position_dodge(0.9),  size=4.0) 
...
ggplot(datos_cw, aes(x=Diagnosis,  y=Total, fill=Diagnosis))+ ggtitle("Diagnóstico de los outliers del atributo Concavity_worst") +
  geom_bar(stat='identity') +                                         
  geom_text(aes(label=Total), vjust=1.6, color="black", position = position_dodge(0.9),  size=4.0) 
...
En la mayoría de los casos (110 de 115), los __outliers__ corresponden a "tumores malignos", por tanto, no es aconsejable eliminar las muestras con esos  valores (ni sustituirlos por otros valores) del conjunto de datos, ya que no parecen ser errores en los datos. Sin embargo, en el caso de los valores __outliers__ de muestras benignas, se podrían haber sustituido por la media de los valores válidos del atributo, ya que sí parecen ser valores erróneos.
