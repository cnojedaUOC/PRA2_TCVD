...
Asimismo, después de analizar los datos se considera que hay que eliminar el atributo **ID**, ya que no aporta información para poder clasificar una muestra de datos como posible benigna o maligna y también se elimina **Diagnosis** porque es nuestra varible objetivo. Así, el nuevo _dataset_ sería el siguiente:
...
col_eliminar <- c("Radius_mean", "Perimeter_mean", "Area_worst", "Radius_worst", "Radius_se", "Permiter_se", "Concave.points_worst", "Concave.points_mean","Diagnosis")

breast_data_def_corr <- breast_data_def[, !(names(breast_data_def) %in% col_eliminar)]
breast_data_def_corr["diagnosis_num"] <- d_num
...
boxplot(scale(breast_data_def_corr[1:5]), xlab ="variables", cex.axis=0.75, ylab = "Rango de Valores", col="blue", main="Información sobre anomalías y valores extremos")
...
IMPORTANTE: se podría comprobar si estos valores outliers corresponde con tumores malignos. Si es así, no se podría eliminar... Dejar para más adelante.
...
Para comprobar cuales de las variables cuantitativas de las que disponemos se empelara la prueba de normalidad de AndersonDarling. Si la variable tiene un p-valor superior a 0.05 se considera que sigue una distribucion normal.  
...
for (i in 1:ncol(breast_data_def_corr)) {
  if (i == 1) cat("Variables que no siguen una distribución normal:\n")
    if (is.integer(breast_data_def_corr[,i]) | is.numeric(breast_data_def_corr[,i])) {
      p_val = ad.test(breast_data_def_corr[,i])$p.value
      if (p_val < alpha) {
        cat(col.names[i])
        # Format output
        if (i < ncol(breast_data_def_corr) - 1) cat(", ")
        if (i %% 3 == 0) cat("\n")
    }
  }
}
...
Puesto que las variables no se ajustan a una distribucion normal, se empleara el coecifiente de correlacion de Spearman para comprobar que variables muestran una mayor relacion con el tipo de tumor. 
corr_matrix <- matrix(nc = 2, nr = 0)
colnames(corr_matrix) <- c("estimate", "p-value")
# Calcular el coeficiente de correlación para cada variable cuantitativa
# con respecto al campo "diagnosis_num"
for (i in 1:(ncol(breast_data_def_corr) - 1)) {
  if (is.integer(breast_data_def_corr[,i]) | is.numeric(breast_data_def_corr[,i])) {
    spearman_test = cor.test(breast_data_def_corr[,i],                 breast_data_def_corr[,length(breast_data_def_corr)],
...
Ordenamos los valores en base a su correlacion, para conocer que variables se correlacionan mas con el diagnostico
corr_matrix[sort(abs(corr_matrix[,1]),decreasing=T,index.return=T)[[2]],]

Se aprecia que las variables que mas influyen son **Perimeter_worst**, **Area_mean**, **Concavity_mean**, **Area_se** y **Concavity_worst**
